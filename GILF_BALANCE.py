...
# Optimize: Use Broadcast Join instead of collect() + isin()
# This avoids bringing all CGL codes to the driver and handles large lists better
df_joined = df_final_schema.join(
...
df_result = df_joined.withColumn(
        "CGL",
        when(col("CGL_NUMBER").isNotNull(), col("CGL")) 
        .otherwise(
...
# Optimize: Cache the result because it is used twice (write to GL_TRANSACTIONS and union for GL_BALANCE)
df_result.cache()
...
final_aggregated_df.coalesce(50).write \
    .format("jdbc") \
    .option("url", oracle_url) \
